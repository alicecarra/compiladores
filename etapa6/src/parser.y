...
item_list -> Result<ASTNode, ParsingError>:
        ...
        global_variable item_list { let declaration_code = $1?;                                  
                                    let node = $2?.extend_code(declaration_code);
                                    Ok(node)
        } |
        function  { $1 }|
        global_variable { 
                let declaration_code = $1?;               
                let node = ASTNode::None(declaration_code);          

                Ok(node)
                };

global_variable -> Result<Vec<AssemblerInstruction>, ParsingError>:
        type name_list ';' {    
                                let variable_type = $1?;        
                                let mut global_declaration_code = vec![];
                                for variable in $2? {
                                        let symbol = SymbolEntry::from_untyped_var(variable, variable_type.clone());
                                        global_declaration_code.extend(generate_global_declaration_code(&symbol));
                                        add_symbol(symbol)?;
                                }
                                Ok(global_declaration_code)
                             };
...
function_call -> Result<ASTNode, ParsingError>:
        ...
        identifier '(' ')'  {
                let identifier = $1?;
                check_declaration(&identifier, $lexer, UsageType::FunctionCall)?;
                let node = FunctionCallCommand::new($span, Box::new(ASTNode::None(Vec::new())), Box::new(identifier))?;
                Ok(ASTNode::FunctionCallCommand(node))
        } ;
...
if->    Result<ASTNode, ParsingError>:
        "TK_PR_IF" '(' expression ')' command_block {
            ...
            let node = IfCommand::new($span, expression, true_command, Box::new(ASTNode::None(vec![])))?;
...
expression6-> Result<ASTNode, ParsingError>:
        ...
        expression6 '/' expression7 {
                let child_left = Box::new($1?);
                let child_right = Box::new($3?);
                let node = Division::new($span, child_left, child_right)?;
...
use etapa6::{ast::{
        ...
        asm::{AssemblerInstruction,generate_global_declaration_code},
        type_enum::{
                Type,
        }};
