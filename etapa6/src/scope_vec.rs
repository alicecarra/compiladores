...
impl ScopeVec {
    ...
    pub fn add_symbol(&mut self, symbol: SymbolEntry) -> Result<(), ParsingError> {
        ...
        match &symbol {
            ...
            SymbolEntry::Variable(content) => {
                scope_table.add_symbol(content.value.clone(), symbol)?
            }
            SymbolEntry::Function(content) => {
                scope_table.add_symbol(content.common.value.clone(), symbol)?
            }
    ...
    pub fn get_register(&mut self) -> Result<String, ParsingError> {
        for table in &mut self.0 {
            if table.scope_type == ScopeType::Function {
                return table.get_register();
            }
        }
